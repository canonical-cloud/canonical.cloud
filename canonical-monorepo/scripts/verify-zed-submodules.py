#!/usr/bin/env python3
"""Enforce non-overlapping Canonical Cloud Zed and submodule ownership."""
...
def repository_from_url(url: str) -> str | None:
    prefixes = (
        "git@github.com:canonical-cloud/",
...
def main() -> int:
    ...
    for section in parser.sections():
        ...
        if repository.endswith(FORBIDDEN_SUFFIXES):
            errors.append(f"forbidden monorepo import: {repository}")
        url_repository = repository_from_url(url)
    ...
    if errors:
        print("Canonical Zed/submodule topology validation failed:", file=sys.stderr)
        for error in errors:
